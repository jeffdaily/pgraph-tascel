...
AS_VAR_PUSHDEF([pt_save_comp], [pt_save_[]_AC_CC[]])
AS_VAR_PUSHDEF([pt_orig_comp], [pt_orig_[]_AC_CC[]])
AS_VAR_PUSHDEF([pt_cv_mpi_naked], [pt_cv_mpi[]_AC_LANG_ABBREV[]_naked])
AC_CACHE_CHECK([for base $wrapped compiler], [pt_cv_mpi_naked], [
base="`$wrapped -show 2>/dev/null | sed 's/@<:@ 	@:>@.*@S|@//' | head -1`"
pt_save_comp="$_AC_CC"
_AC_CC="$base"
AC_LINK_IFELSE([AC_LANG_PROGRAM([],[])], [pt_cv_mpi_naked="$base"])
_AC_CC="$pt_save_comp"
versions="--version -v -V -qversion"
found_wrapped_version=0
# Try separating stdout and stderr. Only compare stdout.
AS_IF([test "x$pt_cv_mpi_naked" = x], [
# prepend any CC/CXX/F77 the user may have specified
compilers="$pt_orig_comp $compilers"
...
do
            ...
            AS_IF([test "x$naked_compiler" != "x$wrapped"],
                [AS_IF([$naked_compiler $version 1>naked.txt 2>naked.err],
                    [AS_IF([$inside mpi.txt naked.txt >/dev/null],
                        [pt_cv_mpi_naked=$naked_compiler; break],
    ...
    AS_IF([test "x$pt_cv_mpi_naked" != x], [break])
done
])
...
AS_IF([test "x$pt_cv_mpi_naked" = x],
    ...
    [AS_IF([test "x$pt_orig_comp" != x && test "x$pt_orig_comp" != "x$pt_cv_mpi_naked"],
        [AC_MSG_WARN([unwrapped $wrapped ($pt_cv_mpi_naked) does not match user-specified $pt_orig_comp])])])
AS_VAR_POPDEF([pt_save_comp])
AS_VAR_POPDEF([pt_cv_mpi_naked])
rm -f inside.pl
])dnl
...
AC_DEFUN([PT_MPI_UNWRAP_PUSH], [
pt_mpi_unwrap_push_save_CC="$CC"
pt_mpi_unwrap_push_save_CXX="$CXX"
pt_mpi_unwrap_push_save_F77="$F77"
pt_mpi_unwrap_push_save_FC="$FC"
AS_IF([test "x$pt_cv_mpic_naked"   != x], [ CC="$pt_cv_mpic_naked"])
AS_IF([test "x$pt_cv_mpicxx_naked" != x], [CXX="$pt_cv_mpicxx_naked"])
AS_IF([test "x$pt_cv_mpif77_naked" != x], [F77="$pt_cv_mpif77_naked"])
AS_IF([test "x$pt_cv_mpifc_naked"  != x], [ FC="$pt_cv_mpifc_naked"])
])dnl
...
AC_DEFUN([PT_MPI_UNWRAP_POP], [
 CC="$pt_mpi_unwrap_push_save_CC"
CXX="$pt_mpi_unwrap_push_save_CXX"
F77="$pt_mpi_unwrap_push_save_F77"
 FC="$pt_mpi_unwrap_push_save_FC"
])dnl
